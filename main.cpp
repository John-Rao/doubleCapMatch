...
//    camera_r.set(CAP_PROP_FRAME_HEIGHT, 480);
    ...
    if (!isVerticalStereo)
    {
        cout<<"is not Vertical Stereo"<<endl;
    ...
    Ptr<StereoSGBM> sgbm = StereoSGBM::create(0, 128, 7,
      10 * 7 * 7,
      40 * 7 * 7,
    ...
    while (1)
    {
      ...
      GaussianBlur(frame_r,frame_r,Size(5,5),0);
      ...
      sgbmDisp16S.convertTo(sgbmDisp8U, CV_16SC1/*, 255.0 / 1000.0*/);
...
//      applyColorMap(sgbmDisp8U, sgbmDisp8U, COLORMAP_HSV);
      ...
      //my test
      Mat ceju=sgbmDisparityShow(Rect(sgbmDisparityShow.cols/2,sgbmDisparityShow.rows/2,3,3));
      double max,min;
      minMaxLoc(ceju,&min,&max);
//      char val=ceju.at<Vec<uchar, 1>>(sgbmDisparityShow.cols/2,sgbmDisparityShow.rows/2)[0];
      cout<<"centre val:"<<1.0/max<<endl;
      sgbmDisparityShow.convertTo(sgbmDisparityShow,CV_8UC1, 255.0 / 1000.0);
      rectangle(sgbmDisparityShow,Rect(sgbmDisparityShow.cols/2,sgbmDisparityShow.rows/2,3,3),Scalar(255));
